...
function maxBy(data, field) {
  const max = data.reduce((max, x, i, arr) => {
    // console.log(x[field] > arr[max]);
    return x[field] > arr[max][field] ? i : max;
  }, 0);
...
function drawChart(data, type, color) {
  ...
  data.map((value, i) => {
    ...
    hoverItem && hoverItem === i && drawHoverPoint(x, y, color);
  });
...
function drawHoverPoint(x, y, originalColor) {
  ctx.closePath();
  ctx.beginPath();
